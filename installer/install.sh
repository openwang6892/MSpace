...
log "获取最新版本信息 ..."
LATEST=$(curl -s https://api.github.com/repos/openwang6892/MSpace/releases/latest | grep -oP '"tag_name": "\K[^"]+')||die "无法获取最新版本"
BASE="https://gh-proxy.org/github.com/openwang6892/MSpace/releases/download/${LATEST}"
raw_base="https://raw.githubusercontent.com/openwang6892/MSpace/refs/heads/main"
log "版本号：${LATEST}"
log "下载 msp 与 color.conf ..."
cd "$TMP_DIR"
wget -q --show-progress "$BASE/msp"||die "下载 msp 失败"
curl -# -o color.conf $raw_base/color/color.conf||die "下载 color.conf 失败"
log "开始安装 ..."
